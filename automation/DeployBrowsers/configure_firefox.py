    ...
    * Privacy extensions
    ...
    if browser_params['ublock-origin']:
        fp.add_extension(extension=os.path.join(
            ...
            'ublock_origin-1.14.11.xpi')
        )
        ublock_dir = os.path.join(storage_dir, 'uBlock0@raymondhill.net')
        if not os.path.isdir(ublock_dir):
            os.mkdir(ublock_dir)

        #Change Storage.js here so the appropriate one is copied to the temp folder
        #Change Storage.js here so the appropriate one is copied to the temp folder
        #Change Storage.js here so the appropriate one is copied to the temp folder
        #print(browser_params['storage_file'])
        shutil.copy(os.path.join(root_dir, 'firefox_extensions',
                                 'ublock_origin', browser_params['storage_file']), ublock_dir+'/storage.js')
